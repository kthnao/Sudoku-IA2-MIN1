...
namespace Sudoku.SolverCSPAIMA
{
    public abstract class SolverCSPAIMA : ISudokuSolver
    {
        ...
        public async Task<SudokuGrid> SolveAsync(SudokuGrid s)
        {
            // S'assurer que Python est installé et configuré
            await PythonInstaller.EnsurePythonInstalledAsync();
            
            // Appel au script Python pour obtenir le CSP de Sudoku
            return await ExecutePythonSolverAsync(s);
        }
        ...
        private async Task<SudokuGrid> ExecutePythonSolverAsync(SudokuGrid s)
        {
            using (Py.GIL())
            {
    ...
    {
        protected override async Task<dynamic> GetStrategy()
        {
        ...
        protected override async Task<SudokuGrid> ExecutePythonSolverAsync(SudokuGrid s)
        {
            using (Py.GIL())
            {
                using (PyModule scope = Py.CreateScope())
